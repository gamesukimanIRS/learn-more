...
class Loader extends PluginBase{
	...
	*/
	...
	public function start($time_target){
		$time_target--;
		if($time_target == 1) $offset = "分";
		else $offset = "分";
		$this->broadcast("§b再起動まで§3 $time_target $offset §fです。");
		if($time_target == 1){
	...
	public function count_down($seconds){
		if($seconds == 1){
			...
			if($seconds == 30) $this->broadcast("§b再起動まで§3 $seconds §f秒です。");
			if($seconds == 10) $this->broadcast("§b再起動まで§3 $seconds §f秒です。");
			if($seconds < 6) $this->broadcast("§b再起動まで§3 $seconds §f秒です。");
